const { allowSpeciesTable, allowedNameTable } = require("../const/const");
...
const IsRulePropertyNameSpeciesEnforced = (rhinoObject, isFilterOff) => {
  ...
  if (properties.length < allowedNameTable.size && isFilterOff) arePropertiesValid = false;
...
const isRuleNameLengthOfCharactersValidEnforced = (rhinoObject, isFilterOff) => {
...
const areAllRulesBroken = (rhino, isFilterOff) => {
  ...
  const isBodyPropertiesCorrect = IsRulePropertyNameSpeciesEnforced(rhino, isFilterOff);
  if(!isBodyPropertiesCorrect)return`Rule Property Name Species is not Enforced.\n 
  ...
  if(!isBodyContentCorrect)return `Rule Name Species Values is not Enforced.\n 
  ...
  if(!isNameNumberOfCharactersValid) return`Rule Name Number Of Characters is not Valid.\n 
